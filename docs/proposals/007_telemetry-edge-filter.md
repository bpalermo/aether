...
- **ABI version pinning.** The `.so` is bound to the Envoy build's ABI hash.
  *Mitigation:* vendor the in-tree SDK at the pinned proxy tag; rebuild the `.so`
  in CI on every proxy bump; the loader rejects a mismatch loudly (fail-closed).
...
`telemetry/edge/` (crate `aether_telemetry`, cdylib) built against the in-tree
