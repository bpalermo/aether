...
func TestCNIMetrics_NilReceiverSafe(t *testing.T) {
	...
	m.sweepCompleted(ctx, 1, 1, 1, 0, 0, 0, 1, nil)
	m.sweepCompleted(ctx, 0, 0, 0, 0, 0, 0, 0, errors.New("boom"))
...
func TestCNIMetrics_SweepCompleted(t *testing.T) {
	...
	m.sweepCompleted(ctx, 2, 1, 3, 4, 5, 2, 7, nil)
	...
	if got, _ := metricSum(t, reader, "aether.agent.ghost_sweep.unmeshed_pods"); got != 7 {
		t.Errorf("unmeshed_pods = %d, want 7 (missingStorage 5 + staleRunning 2)", got)
	}
...
func TestCNIMetrics_SweepFailed(t *testing.T) {
	...
	m.sweepCompleted(context.Background(), 0, 0, 0, 0, 0, 0, 0, errors.New("registry down"))
