...
install_aether() {
	...
	for c in "$CLUSTER_A" "$CLUSTER_B"; do
		...
		helm --kube-context "kind-$c" upgrade --install aether "$charts/aether" -n "$NS" --create-namespace \
			...
			$(img agent agent) $(img agent.meshDnsDaemon mesh-dns) $(img proxy.supervisor proxy-supervisor) $(img cniInstall cni-install) $(img registrar registrar) $(img controller controller) \
