...
- **`registry/`** - Service registry interface with DynamoDB (`internal/ddb/`), etcd (`internal/etcd/`), and registrar (`internal/registrar/`) implementations. The agent selects the backend via `--registry-backend` flag. Manages service endpoint registration and discovery.
