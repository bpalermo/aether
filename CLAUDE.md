...
- **`controller/internal/`** - The controller's webhooks and reconciler: `webhook/` dispatches the `/validate` admission endpoint by Kind to `meshconfig/`, `httpfilter/`, `edgeconfig/` (edge best-practices/HTTP3 hardening, proposal 029), and `gatewayapi/` validators; `podmutate/` is the `/mutate` pod-mutating webhook (ndots + namespace mesh injection); `meshconfig/` reconciles each namespace's `MeshConfig` CR into a projected ConfigMap.
...
- **`api/`** - Protobuf definitions under `aether/cni/v1/`, `aether/registry/v1/`, `aether/registrar/v1/`, and `aether/config/v1/` (`MeshConfig`, `HTTPFilter`, `EdgeConfig`). Uses `buf/validate` for proto validation and `protoc-gen-dynamo` for DynamoDB marshaling.
- **`common/apis/config/v1/`** - Kubernetes CRD Go types (`MeshConfig`, `HTTPFilter`, `EdgeConfig`) wrapping the `aether/config/v1` protos with deepcopy/jsonshim glue.
